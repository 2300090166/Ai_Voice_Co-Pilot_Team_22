...
import { Mic, MicOff, PhoneOff, ShieldCheck, Sparkles, CheckCircle2, Send, Bot, FileText, Check } from 'lucide-react';
 ...
 * dynamic AI Suggestions panel (Next-Best Actions), RAG context, and session termination.
 */
export default function CallAssistant() {
  ...
  const [currentResponse, setCurrentResponse] = useState(sampleResponse);
  ...
  const handleEndSession = async () => {
    try {
      ...
      console.warn('Call end API warning:', err);
    }
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center space-x-2">
              ...
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                ...
                {sessionEnded ? 'Ended' : isLive ? 'Streaming Live' : 'Standby'}
              </span>
            </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            ...
            className="flex items-center space-x-2 px-4 py-2 rounded-lg font-bold text-xs bg-red-500/10 text-red-400 border border-red-500/30 hover:bg-red-500/20 transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          >
          ...
          <Card title="Live Call Transcript Feed" subtitle="Whisper STT Stream & Turn Processing" className="flex-1 flex flex-col min-h-[500px]">
            <div className="flex-1 overflow-y-auto space-y-3 pr-1 max-h-[380px]">
              {transcripts.length === 0 ? (
                <div className="p-8 text-center text-slate-500 space-y-2 border border-dashed border-slate-800 rounded-xl">
                  ...
                  <p className="text-xs font-semibold">Call session has ended.</p>
                  <p className="text-[11px] text-slate-600">Click "Start Co-Pilot Stream" to begin a new customer conversation.</p>
                </div>
              ) : (
            ...
            <form onSubmit={handleSendTurn} className="mt-4 pt-3 border-t border-slate-800 flex items-center space-x-2">
              <input
                ...
                placeholder={sessionEnded ? 'Session ended. Start stream to type query...' : "Simulate customer utterance (e.g., 'What KYC documents do I need?')..."}
              ...
              <button
                type="submit"
                disabled={loading || sessionEnded || !inputQuery.trim()}
